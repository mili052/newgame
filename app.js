...
const DATA_PATHS = ["./kanban.json", "./文档/kanban.json", "./docs/kanban.json"];
const REPORT_PATHS = ["./report.md", "./文档/report.md", "./docs/report.md"];
    ...
    const fetchFirstOk = async (paths) => {
      ...
      for (const p of paths) {
        try {
          ...
          if (res.ok) return { path: p, res };
          lastErr = `${res.status} ${res.statusText}`;
        } catch (e) {
          lastErr = e?.message || String(e);
        }
      }
      throw new Error(`尝试路径：${paths.join("、")}；最后错误：${lastErr}`);
    };

    const [{ res: dataRes }, { res: reportRes }] = await Promise.all([fetchFirstOk(DATA_PATHS), fetchFirstOk(REPORT_PATHS)]);
